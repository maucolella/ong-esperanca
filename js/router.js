...
import { buscarInteresses, salvarUltimaRota } from "./storage.js";

const rotas = {
    inicio: templateInicio,
    projetos: () => templateProjetos(buscarInteresses()),
...
export function renderizarPagina({ manterFoco = false } = {}) {
    const app = document.getElementById("app");
    const anuncio = document.getElementById("anuncioRota");
    const rotaSolicitada = obterRotaAtual();
    const rota = rotas[rotaSolicitada] ? rotaSolicitada : "inicio";
    ...
    document.title = `${titulos[rota]} - ONG Esperança`;
    anuncio.textContent = `Página ${titulos[rota]} carregada.`;
