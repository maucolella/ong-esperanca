import { mostrarAlertaSucesso, mostrarToast } from "./feedback.js";
...
function aplicarMascara(campo) {
    ...
    if (campo.id === "telefone") {
        const valor = numeros.slice(0, 11);
        campo.value = valor
            .replace(/^(\d{2})(\d)/, "($1) $2")
...
function atualizarEstadoCampo(campo) {
    ...
    campo.classList.toggle("campo-sucesso", valido && campo.value !== "");
    campo.classList.toggle("campo-erro", !valido);
    campo.setAttribute("aria-invalid", String(!valido));

    if (erro) {
        erro.textContent = valido ? "" : (mensagens[campo.id] || "Verifique este campo.");
    }
...
export function validarFormulario(formulario) {
    ...
    campos.forEach(campo => {
        const valido = atualizarEstadoCampo(campo);
...
export function tratarInput(campo) {
...
export function limparValidacao(formulario, ocultarAlerta = true) {
    ...
    const alerta = document.getElementById("mensagemSucesso");
    if (alerta && ocultarAlerta) {
        alerta.hidden = true;
    }
}

export function processarCadastro(formulario) {
    if (!validarFormulario(formulario)) {
        mostrarToast("Revise os campos destacados antes de enviar.");
        return false;
    }

    // Dados pessoais não são persistidos no localStorage.
    mostrarAlertaSucesso();
    mostrarToast("Obrigado por se cadastrar na ONG Esperança!");
    formulario.reset();
    limparValidacao(formulario, false);
